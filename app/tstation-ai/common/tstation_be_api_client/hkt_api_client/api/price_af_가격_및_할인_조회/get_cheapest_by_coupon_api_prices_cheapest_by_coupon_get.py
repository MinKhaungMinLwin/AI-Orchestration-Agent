...
def sync_detailed(
    *,
    ...
    """회원 보유 쿠폰 기반 상품별 최저가 시뮬레이션
     ...
     회원 보유 쿠폰을 상품→결제→서비스→플러스 순으로 그리디 적용하여 상품별 최종 혜택가를 계산합니다. 각 단계에서 적용 가능한 쿠폰 중 할인금액이 가장 큰 1개를 자동 선택합니다.
    상품쿠폰의 CPN_DUP_USE_YN='N' 이면 결제쿠폰 단계를 건너뜁니다. 서비스쿠폰과 플러스쿠폰은 상품/결제쿠폰과 독립적으로 최대 1개씩 적용합니다. 여러 상품을 받아 각각
    독립적으로 시뮬레이션합니다(여러 상품 중 1개를 고르는 것이 아님). 회원번호/제휴사번호는 JWT 클레임에서, 매장 ID 와 채널은 query 로 받습니다.
