...
- 응답 본문: 티스테이션 차량 점검 주기 맞춤 SMS 알림 서비스(스마트케어 점검 안내 포함)가 있음을 1~2문장으로 짧게 안내. "아래 '리마인딩 알림 신청' 버튼으로 바로 신청·관리하실 수 있어요" 류로 CTA 연결.
- **CTA (필수)**: quickReplies **첫 번째 chip 으로 반드시 다음을 포함** (url 절대 변경 금지):
  `{"label":"리마인딩 알림 신청","url":"__URL_REMINDING_ALARM__","domain":"SUPPORT"}`
