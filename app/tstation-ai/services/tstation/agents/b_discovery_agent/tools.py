...
def search_product_tool(keyword: str | None = None, limit: int = 5, size: str | None = None, brand_cd: str = "HK"):
    ...
    Args:
        keyword (str | None): 검색할 제품명 키워드 (Korean preferred). 브랜드명만이면 None.
        limit (int): 반환할 최대 상품 수. Default: 5.
        size (str | None): 타이어 사이즈 필터 (e.g., '225/45R17'). Optional.
        brand_cd (str): 브랜드 코드 (see above). Default: HK.

    Examples:
        - {"keyword": "벤투스 S2", "limit": 5, "size": "225/45R17"}
        - {"size": "235/55R19", "brand_cd": "BS"}
    """
...
def get_products_recommendations_tool(rcmd_type: RcmdType, limit: int = 5, brand_cd: str = "HK", car_lnc_cd: str | None = None, tire_size: str | None = None):
    ...
    Args:
        rcmd_type (RcmdType): Recommendation type (see above).
        limit (int): Number of products to return. Default 5, max 100.
        brand_cd (str): Brand code — HK=Hankook (default), LF=Laufenn, MC=Michelin, PI=Pirelli, BS=Bridgestone, CT=Continental, GY=Goodyear.
        car_lnc_cd (str | None): 차량 런칭 코드 — takes priority over tire_size when provided.
        tire_size (str | None): 타이어 사이즈 (e.g., "245/45R18").

    Examples:
        - {"rcmd_type": "tstation", "limit": 10, "brand_cd": "HK"}
        - {"rcmd_type": "wet", "limit": 5, "tire_size": "245/45R18"}
    """
