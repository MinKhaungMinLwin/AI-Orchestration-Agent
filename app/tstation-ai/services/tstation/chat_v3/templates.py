...
class _AnswerUiSignals(BaseModel):
    requests_store_selection: bool = Field(
        description=(
            "이 챗봇 답변이 사용자에게 장착 매장 또는 지역을 선택하거나 알려달라고 요청하고 있는지 여부. "
            "여러 후보 중 고르라는 경우뿐 아니라, 봇이 특정 매장/지역 하나를 제안하며 그것으로 진행할지 "
            "확인/응답해 달라고 요청하는 경우도 포함한다."
        )
    )
    requests_datepick: bool = Field(
        description=(
            "이 챗봇 답변이 사용자에게 설치 희망 날짜 또는 시간을 선택하거나 알려달라고 요청하고 있는지 여부. "
            "여러 날짜/시간 후보 중 고르라는 경우뿐 아니라, 봇이 특정 날짜/시간 하나를 제안하며 그 일정으로 "
            "진행할지 확인/응답(특정 문구로 말해달라는 요청 포함)해 달라고 요청하는 경우도 포함한다."
        )
    )
    asks_quantity: bool = Field(
        description=(
            "이 챗봇 답변이 사용자에게 주문/구매 수량을 선택하거나 알려달라고 요청하고 있는지 여부. "
            "여러 수량 후보 중 고르라는 경우뿐 아니라, 봇이 특정 수량 하나를 제안하며 그 수량으로 진행할지 "
            "확인/응답해 달라고 요청하는 경우도 포함한다."
        )
    )
