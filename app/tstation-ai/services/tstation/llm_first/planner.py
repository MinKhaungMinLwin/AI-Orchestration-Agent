    ...
    async def plan(
        ...
        *,
        ...
        prompt = (
            ...
            "If the user asks for recommendation/recommended picks, do not return a comparison table even when multiple product names appear; "
            "select ProductRecommendationAF so the response can be product cards or product-by-product recommendation text. "
            "During an active purchase flow, requests like '다른 타이어', '다른 매장', or '다른 지역' mean show alternatives excluding "
            "the currently selected product/store; set known_inputs.alternative_target to product or store and preserve the existing size/quantity context. "
