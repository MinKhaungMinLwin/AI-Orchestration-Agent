...
class RouterEntities(BaseModel):
    ...
    Domain agents read these to skip their own heuristic re-classification.
    """
...
class _SlimMultiAgentDomain(BaseModel):
    ...
    intent / entities are KEPT on first turn — they are the primary
    routing signal even on a single message ("내 제타에 전기차용 껴도 돼?"
    needs intent=COMPATIBILITY at turn 1, not turn 2).
    """
...
def prompt_router_slim() -> str:
    ...
    primary routing signal that lets domain agents skip their own heuristic
    re-classification. See INTENT TAXONOMY and ENTITY EXTRACTION below.
    """
    return """
You are a domain classifier for T-Station AI (Hankook Tire).
...
class StreamingMultiAgentCoordinator:
    ...
    def _inject_conversation_context(messages: list[dict], routing: MultiAgentDomain | None) -> list[dict]:
        ...
        Domain agents read the injected `Intent` line as their primary dispatch signal —
        this replaces the heuristic re-classification each agent prompt used to do.
