    ...
    *,
    ...
    if intent == "tire_condition_photo_policy" or _TIRE_CONDITION_PHOTO_POLICY_RE.search(text):
        return _decision(
            ...
            assistant_guidance=(
                "타이어 상태를 사진으로 봐달라는 문의는 FAQ hybrid 검색을 먼저 수행하되, 사용자가 사진/파일 업로드나 첨부를 말하면 "
                "현재 챗봇에서는 업로드 확인이 불가능하다고 먼저 안내한다. 사진만으로 마모 상태, 교체 필요 여부, 주행 안전을 확정할 수 "
                "없다고 분명히 말하고, 가까운 매장 점검이나 전문 점검으로 마모도/손상 여부 확인을 우선 안내한다."
            ),
        )
