...
def _get_num(d: dict, *keys: str, default: int | float = 0) -> int | float:
    for k in keys:
        ...
        if v is not None:
            try:
                return type(default)(str(v).replace(",", ""))
            except (ValueError, TypeError):
                pass
    return default


def _display_final_unit_price(row: dict) -> int:
    """FINAL unit price for product cards/comparison, not order payment total."""
    for key in (
        "cheapest_final_prc",
        "final_unit_price",
        "final_prc",
...
def _map_product(tool_data_list: list[dict], assistant_text: str) -> dict | None:
    ...
    #
    # Price priority: cheapest_final_prc → final_unit_price/final_prc →
    # extra_fvr_sale_prc → price → sale_prc. This is display/comparison FINAL
    # unit price; order payment totals add wage and quantity in their own path.
    ...
    for entry in _find_entries(tool_data_list, "get_final_price_tool"):
        ...
        if not isinstance(price_data, dict):
            continue
        price = _display_final_unit_price(price_data)
        if price:
    ...
    for entry in _find_entries(tool_data_list, "get_my_cars_tool", "get_user_vehicles_tool"):
        ...
        for row in rows:
            ...
            metadata.append({
                ...
                "mbrCarUnifNo": _get_str(row, "mbr_car_unif_no", "mbr_car_reg_seq") or None,
                "mbr_car_unif_no": _get_str(row, "mbr_car_unif_no", "mbr_car_reg_seq") or None,
                "carMaker": _get_str(row, "car_maker") or None,
                "carNm": _get_str(row, "car_nm") or None,
...
def _map_cheapest_product(tool_data_list: list[dict], assistant_text: str) -> dict | None:
    ...
    for row in rows:
        ...
        final_price = _display_final_unit_price(row)
...
def _map_runflat_price_comparison(tool_data_list: list[dict], assistant_text: str) -> dict | None:
    ...
    for price_row in price_rows:
        ...
        if not product_row:
            continue
        final_unit_price = _display_final_unit_price(price_row)
...
def _map_order_complete(tool_data_list: list[dict], assistant_text: str) -> dict | None:
    ...
    # with FINAL resolved as cheapest_final_prc → final_unit_price/final_prc →
    # extra_fvr_sale_prc → price → sale_prc.
    payment_amount: int | None = None
    for price_entry in _find_entries(tool_data_list, "get_final_price_tool"):
        praw = _unwrap(price_entry)
        if not isinstance(praw, dict):
