...
def test_order_preview_generic_quickreply_is_coerced_to_datepick() -> None:
    ...
    assert result["data"]["assistantResponse"] == "티스테이션 한남점에서 예약하려는 날짜와 시간을 선택해 주세요."
    assert result["data"]["metadata"] == {
        "shopId": "F07782",
        "shopName": "티스테이션 한남점",
    }
...
def test_store_scope_request_does_not_reuse_latest_datepick() -> None:
    latest_datepick = {
        "assistantResponse": "티스테이션 한남점 예약 가능한 시간을 확인했어요.",
        "dates": [{
            "date": "2026년 6월 24일 (수)",
            "available": True,
            "availableTimes": [17],
    ...
    event = {
        ...
        "source_domain": "transaction",
        "data": {
            "assistantResponse": "내일 장착 가능한 성남 지역 매장으로 다시 확인해 드릴게요.",
            "quickReplies": [],
        },
    }
